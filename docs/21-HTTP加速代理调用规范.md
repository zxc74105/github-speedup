...
| 模式 | 说明 | 谁在用 |
|------|------|--------|
| **主动下载**（GUI） | 桌面 GUI 操作，Python 多代理并行分片（主）+ aria2 直连镜像（兜底） | 终端用户 |
| **被动加速**（HTTP 服务） | 端口 9090 HTTP 透明代理 | 浏览器/curl/aria2/脚本等**外部工具** |

> **重要**：9090 HTTP 加速代理是给**外部工具**用的，**不是**给软件自身组件用的。软件自身下载组件（aria2、Python 分片）直接连接镜像代理网址（`https://镜像域名/https://github.com/...`），不经过 `http://127.0.0.1:9090/...`。
...
请求到达
  │
  ├── 遍历代理列表（Status="可用", Enabled=true, Scheme!=""）
  │   ├── 依次尝试
  │   │   └── proxy_request() → 成功? → 返回
  │   └── 失败 → 下一个
...
[2026-07-26 12:00:01.500] SUCCESS gh-proxy.com - 1048576 bytes, 1.2s
```
